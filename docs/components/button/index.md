...
<div style="margin-bottom:20px;">
    <SButton color="blue" plain>朴素接钮</SButton>
    <SButton color="green" plain>绿色按钮</SButton>
    <SButton color="gray" plain>灰色接钮</SButton>
...
带图标的按钮可以增强辨识度（有文字）或节省空间（无文字）。

:::demo 设置 icon 属性即可，icon 的列表可以参考 Element 的 icon 组件，也可以设置在文字右边的 icon，只要用 i 标签即可，可以使用自定义图标。
...
<template>
    <div class="flex flex-row">
        ...
        <SButton icon="search" round plain>搜索</SButton>
    </div>
</template>
```